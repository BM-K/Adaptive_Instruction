...
class Beam:
    ...
    def __init__(self, beam_size=5, opt=None,
        ...
        self.current_scores = [0] * beam_size
    ...
    def advance(self, next_log_probs, cur_idx=0):
        ...
        if self.isfirst_step:
            ...
            for i in range(len(top_scores)):
                self.current_scores[i] = float(top_scores[i].data.cpu().numpy())
            ...
            # next_log_probs = [beam_size x cur_idx+1 x vocab_size] | ex) [5, 1+1, 50000]
            for idx in range(len(next_log_probs)):
                required_idx.append(cur_idx)
            ...
            for idx in range(len(required_idx)):
                if self.finished_beam[idx] == True:
                    ...
                    score, ids = next_log_probs[idx][required_idx[idx]].topk(k=1, dim=0)
                    temp_current_score[idx] = float(score[0].data.cpu().numpy())
